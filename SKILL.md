...
description: >-
  ...
  새 글은 korean-writing 워크플로우로 연결하고, 기존 원고는 사실·인용·숫자·화자를 보존하면서 14개 거시 패턴, 장문 무손실 청킹, 4축 검증 게이트를 적용해 윤문한다. 구조 진단·완독률 개선·칼럼 고도화 요청에서는 Editorial Audit을 분리 실행한다. 단순 맞춤법 교정이나 원문에 없는 내용을 추가하는 작업에는 사용하지 않는다.
---

# 한글 글쓰기(윤문) — 진단 증거·작성 의도·보존 계약 스킬 v10.1
...
- `--audit`·`--editorial`·`--full-edit`, “구조까지”, “완독률”, “칼럼 고도화” 요청에서는 [`references/editorial-audit.md`](references/editorial-audit.md)를 읽는다.

## 1. 네 가지 철칙

1. **의미 불변:** 사실, 주장, 수치, 날짜, 고유명사, 직접 인용, 법조문, 표·불릿의 항목을 보존한다.
2. **탐지된 구간만 수술:** 기본 윤문은 `references/quick-rules.md` 또는 `scripts/patterns.json`에 매핑된 span만 고친다. `--full-edit`의 구조 이동은 `references/editorial-audit.md` 계약에 따르며, 원문에 없는 사실·주장·인과를 만들지 않는다.
...
| `--audit`·편집 진단만 | Editorial Audit | 한 문장 주장·결론 선배치·전제 누락·개인 반복·이탈 지점·리듬·글 분리를 진단하고 본문은 바꾸지 않음 |
| `--editorial`·구조까지·완독률·칼럼 고도화 | 편집 설계 | Editorial Audit + 적용 순서·변경 예산 제시. 거시 구조는 유지 |
| `--full-edit`·구조 재배치까지 | 구조 편집+윤문 | 진단 → 재배치 기준본 → 국소 윤문 → 구조·LOCK 보존 검증 |
...
Editorial Audit은 윤문 강도와 별개의 거시 편집 레이어다. 개인 반복은 문서별 습관으로 다루며 AI 작성 신호에 합산하지 않는다. `--audit`은 진단만, `--editorial`은 진단과 편집 계획, `--full-edit`은 재배치 기준본을 만든 뒤 국소 윤문까지 수행한다. 세부 진단·출력·보존 계약은 `references/editorial-audit.md`를 따른다. “최강 윤문”은 heavy 경로와 Editorial Audit을 함께 실행하되, 구조 변경 의사가 명시되지 않으면 문단 순서를 유지한다.
...
현재 스킬의 `scripts/patterns.json`을 운영 SSOT로 사용한다. A 번역투·B 관공서 상투구·C 형식명사·D 부호·E 구조·F 리듬·G 과장·H 수동·I 장식·J 접속어·K 감정 표방·L 챗봇 흔적·M 헤지 스택·N 자기 라벨 인플레·O 사용자 금지 어휘를 P0/P1/P2로 분류한다.
...
- **O 사용자 금지 어휘:** `갈래`(세 갈래로 나뉜다, 첫 갈래는)는 실제 독자가 잘 쓰지 않는 말이므로 윤문본·새 글 어디에도 쓰지 않는다. `세 가지로 나뉜다`, `첫 번째는`, `유형`처럼 일상어로 바꾼다. 직접 인용·고유명사 안의 `갈래`만 LOCK으로 유지한다.
...
대화형 요청에서는 윤문본과 함께 `완료. 변경률 X% / 등급 Y / 자체검증 N/6` 한 줄, 실제 수정한 category 3~6개, 핵심 변경 1개, 보류·경고를 간결하게 보고한다. `--audit`은 `references/editorial-audit.md`의 진단 형식만 출력하고, `--full-edit`은 이동 문단과 재배치 기준본 대비 변경률을 함께 보고한다. 사용자가 요청하지 않은 SNS 요약·문서 변환·제목 후보는 만들지 않는다.
...
- [ ] Editorial Audit이면 개인 반복을 AI 작성 신호로 오판하지 않았는가
- [ ] `--full-edit`이면 문단 대응표·재배치 기준본·구조 보존 검증을 남겼는가
