...
## 10.0.0 (2026-08-28) 10차 고도화: "진단 증거·작성 의도·보존 계약"

- Detect 2.0: 범주 횟수에 실제 span, 행·문단, layer, 명백/문맥 판단, 유지 조건, 조치를 추가했다.
- AI 저자 판정과 편집 신호를 분리하고 사용자 출력의 “AI 흔적 지수·사람 결”을 “편집 신호 밀도·낮음”으로 바꿨다.
- 인용·코드·YAML·표·URL·각주를 진단에서 면제하고 윤문 전후 문자열 단위로 검증한다.
- 수정 후 새 P0/P1 패턴 증가를 보존 게이트 실패로 처리한다.
- context 8종과 voice 6종을 독립 축으로 정의하고 윤기자 voice를 선택형으로 전환했다.
- Writer–Editor JSON 인계 계약, `handoff-validate`, 구조 감사, 선택형 `kiwipiepy` 형태소 보조 명령을 추가했다.
- 단일 `것이다`, `~를 통해`, 대명사, 단일 접속 부사의 오탐을 실측 유지 조건에 맞춰 완화했다.
- 누락됐던 글쓰기 참고 문서를 복구하고 고정 슬롯·감성 레이어 강제를 선택 구조로 바꿨다.
- 장르별 false-positive fixture, 저장소 self-scan, 참조·버전·내장 자산 검증, 재현 가능한 `.skill` 빌드를 CI에 추가했다.

## Unreleased

- `neo-korean-writing` 설치형 CLI와 `pyproject.toml`을 추가했습니다. 내장 프롬프트·템플릿 조회, 장르별 작업공간 초기화, 문체 진단, 원문·수정본 보존 대조, 번역문 감사를 제공합니다.
- Python wheel·source distribution을 빌드하고 격리 환경 설치를 검증할 수 있도록 CLI 사용·배포 문서 `docs/cli.md`와 CLI 회귀 테스트를 추가했습니다.

- 홈페이지에 한글 글쓰기(윤문)의 적용 범위와 책임 있는 윤문 가이드라인을 추가했습니다. 사실 LOCK, 장르·문체 보존, 근거 없는 추가 금지, 사람의 최종 판단을 명시했습니다.
- `prompts/`에 범용 윤문, 보도자료·공식 안내문, 장문 원고, 번역문 후편집 프롬프트를 추가했습니다.
- `templates/`에 작업 의뢰서, LOCK 보존 항목 대조표, 결과 전달서와 사용 안내를 추가했습니다.
- README와 SKILL 작업 라우터에서 프롬프트·템플릿 선택, 보존 검증, 결과 전달 흐름을 연결했습니다.
...
윤기자 설계의 최근 업데이트(v2.2 route_hint 재편, v2.3 구조 수렴 게이트·슬림 진단·대조 코퍼스 실증 백포트)를 반영했다.
...
윤기자의 윤문 설계 자료와 한국번역학계 유형을 참고해 윤문 본체와 재현 도구를 확장했다.
