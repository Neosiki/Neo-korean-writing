...
`connectives`는 후보만 제시하며 자동 삭제를 기본값으로 삼지 않는다. `--remove-redundant`를 지정하면 같은 표지가 반복되거나 여러 접속 표지가 밀집한 경우에만 축약한 `rewritten` 결과를 함께 출력한다. 결과를 채택하기 전에 반전·인과·양보·범위가 유지되는지 사람이 확인한다.
