...
# Codex invokes skills from the user's current workspace. Keep run artifacts
# there rather than writing them into the installed skill directory.
...
def _resolve_run_dir(run_dir_arg: str | None, text_arg: str | None) -> Path:
    ...
    workspace = CALLER_ROOT / "_workspace"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.diagnosis:
        ...
        if not diag_path.is_absolute():
            diag_path = CALLER_ROOT / diag_path
