...
`bf4c9ce`, `main` 통합 커밋은 `6dbcc92`이며, feature 브랜치는 통합 후 삭제되었다. v8은 `epoko77-ai/im-not-ai`
v2.2~v2.3의 성과를 이식한 구현 확장으로, 4축 구조 수렴 게이트(`verify_gates.py`), 진단 전용 슬림 인덱스
(`diagnosis-rules.md`), 대조 코퍼스 실측에 근거한 규칙 심각도 조정(C-8 승격, A-2·I-1 완화, A-16 범위 축소, E-1
재정의)을 담는다. v8이 인용하는 대조 코퍼스 수치(예: 부정 대구 밀도 AI 5.8 대 인간 0.6)는 상류 저장소의 측정이며 본
연구가 독립 재측정한 값이 아니다. v8 검증은 결정적 코드 검사(빌드 무결성·회귀 테스트·게이트 동작·상류 구현과의 판정
일치)에 한정된다.
...
확장하고, 진단이 분류표 전량 대신 읽는 결정적 생성물인 슬림 인덱스를 도입했으며, 상류 저장소의 대조 코퍼스 실측을 근거로
일부 규칙의 심각도를 승격·강등해 과잉교정 가드를 강화했다. v8의 검증은 회귀 테스트, 생성물 무결성 검사, 합성 사례에 대한
게이트 동작 확인, 상류 구현과의 판정 일치 확인으로 구성된 결정적 코드 검증이며, 새로운 코퍼스 실측이나 인간 평가가 아니다.
...
`epoko77-ai/im-not-ai`의 `scholarship.md`는 단일 연구가 모든 현상을 포괄한다고 주장하지 않고, 한국어 번역투 연구와
...
v8은 상류 저장소 `epoko77-ai/im-not-ai`의 실측 사례에서 출발한다. 문자 변경률 2.77%로 "수렴"으로 판정된 윤문본의
...
심각도 조정은 상류의 대조 코퍼스 실증(`references/empirical-validation.md`)을 근거로 한다. C-8 부정 대구는 AI에서
...
C-8 전멸(5→0)을 FAIL로 잡아 전체 판정을 경고로 올리는 것을 확인했다. 넷째, 동일 입력에 대해 상류 저장소의 원본
`verify_gates.py`와 이식본이 같은 exit code를 반환하는 판정 일치를 확인했다. 이는 이식의 충실성과 게이트의 결정적
...
epoko77-ai. (2026). *im-not-ai: Humanize Korean references and scholarship
taxonomy*. GitHub repository. https://github.com/epoko77-ai/im-not-ai
...
아래 표는 `epoko77-ai/im-not-ai`의 `references/scholarship.md`와 `ai-tell-taxonomy.md`를 기준으로 작성한 설계 매핑이다.
